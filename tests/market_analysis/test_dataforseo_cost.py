...
def _enabled_provider(monkeypatch, tmp_path):
    ...
    # Isolated cache DB per test so cache state never leaks between tests.
    cache_db = tmp_path / "cache.db"
    init_db(cache_db)
    return DataForSEOProvider(cache_db_path=cache_db)


def test_search_volume_endpoint_skipped_by_default(monkeypatch, tmp_path):
    monkeypatch.delenv("DATAFORSEO_SEARCH_VOLUME_ENABLED", raising=False)
    provider = _enabled_provider(monkeypatch, tmp_path)
...
def test_search_volume_endpoint_used_when_enabled(monkeypatch, tmp_path):
