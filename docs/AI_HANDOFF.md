...
- **Open issues:** Étapes manuelles requises côté Adrien avant que l'app refonctionne : (1) ajouter le custom domain `pilot.leoniedelacroixfrance.com` sur le service Render `leonie-seo-pilot-web` + CNAME DNS sur `leoniedelacroixfrance.com` ; (2) une fois `https://pilot.leoniedelacroixfrance.com/healthz` répond, push ce diff (déploiement auto Render) ; (3) exécuter `shopify app deploy` depuis `shopify-app/` pour synchroniser `application_url`/`redirect_urls` avec le Partner Dashboard Shopify ; (4) retirer l'ancien custom domain `pilot.leoniedelacroix.com` de Render (optionnel). Si le domaine du storefront marchand (`www.leoniedelacroix.com`) a aussi changé, c'est un suivi séparé (impacte `config/tenants/leoniedelacroix.yaml`, GSC property).
