...
- **Goal:** Feature « Fichiers IA » (agents.md / llms.txt / llms-full.txt) publiables en 1 clic depuis le dashboard, mis à jour automatiquement sur changement catalogue.
- **Summary:** Générateur **déterministe** (zéro LLM) du contenu à partir du snapshot Shopify + business profile (`app/geo/llms_txt.py` : `build_llms_txt`, `build_llms_full_txt`, `build_agents_md`, `build_llms_payload`, `wrap_liquid_raw`, helpers partagés extraits dans `app/geo/_shared.py`). **Pivot d'architecture majeur** : Shopify sert désormais `/llms.txt`, `/llms-full.txt`, `/agents.md` nativement via des templates de thème ([changelog 28 mai 2026](https://shopify.dev/changelog/customize-llmstxt-llms-fulltxt-and-agentsmd)) — l'approche initiale Files API + URL Redirect (livrée puis abandonnée) est **silencieusement ignorée** par Shopify. Le mécanisme de publication écrit maintenant les 3 `templates/*.liquid` sur le thème publié via `themeFilesUpsert` (2025-01), enveloppés dans `{% raw %}` (anti-injection Liquid). Publish idempotent par hash ; unpublish = `themeFilesDelete` (retour défaut Shopify) ; régénération auto debouncée 5 min sur webhooks catalogue. **Changement de scope OAuth** : `read_themes` + `write_themes` ajoutés (re-consent marchand requis au déploiement). UI : page dédiée `app.geo-llms-txt.tsx` + panneau `LlmsTxtPanel` sur l'accueil + entrée menu + i18n FR/EN.
- **Files created:** `app/geo/_shared.py`, `app/geo/llms_txt.py`, `app/apply/shopify_theme_files.py`, `app/llms_txt/{__init__,store,publisher}.py`, `app/api/llms_txt.py`, `shopify-app/app/routes/app.geo-llms-txt.tsx`, `shopify-app/app/components/LlmsTxtPanel.tsx`, tests (`tests/test_geo/test_llms_txt.py`, `tests/apply/test_shopify_theme_files.py`, `tests/test_llms_txt/test_{store,publisher}.py`, `tests/test_api/test_llms_txt_api.py`).
- **Files modified:** `app/geo/crawlability.py` (réutilise `_shared`), `app/db.py` (table `llms_txt_publications`), `app/main.py` (router), `shopify-app/app/routes/{app.tsx,app._index.tsx,webhooks.tsx}`, `shopify-app/app/lib/i18n.ts`, scopes (`shopify.app{,.pilot,.local}.toml`, `render.yaml`, `.env.example` ×2), `shopify-app/shopify.app.toml` (webhooks catalogue), `PROGRESS.md`.
...
- **Decisions made:** 100% déterministe (anti-hallucination). Écrire les 3 templates (choix utilisateur) pour contrôle total des routes. agents.md v1 = même index que llms.txt. Review `shopify-safety` passée sur l'ancien flow avant le pivot. Scope `write_content` conservé (blog) ; `write_themes` est le bon scope depuis que Shopify a fait des templates de thème la méthode officielle.
- **Validations run:** `ruff check` ✅ ; `pytest tests/test_geo/test_llms_txt.py tests/apply tests/test_llms_txt tests/test_api/test_llms_txt_api.py` → **114 passed** ; `cd shopify-app && npm run typecheck && npm run build` ✅.
- **Validations skipped:** Pas de test live boutique de dev — nécessite `shopify app deploy` + re-consent `write_themes` (cf. checklist Phase 12 de `PROGRESS.md`). Capture embedded non produite (nécessite tunnel `npm run dev`).
- **Open issues:** (1) Re-consent marchand requis avant que publish fonctionne (sinon 403). (2) Templates liés au thème publié au moment du publish ; un changement de thème nécessite une re-publication (webhook `themes/publish` = amélioration future). (3) `themeFilesUpsert` async (`job`) non pollé — on se fie à `upsertedThemeFiles`. (4) Aucun commit git créé.
- **Next recommended action:** `shopify app deploy` + réinstaller la boutique de dev → publier depuis `/app/geo-llms-txt` → vérifier les 3 templates dans Online Store > Themes > Edit code + `curl https://{shop}/llms.txt|/agents.md|/llms-full.txt` (200) → tester la régénération webhook + le « Dépublier ».
