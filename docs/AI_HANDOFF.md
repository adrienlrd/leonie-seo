...
- **Decisions made:** Nom de l'app (`Organically` dans `.toml` vs `Léonie SEO` dans la doc) **laissé inchangé** — décision utilisateur « nom à valider » (vérifier l'unicité App Store avant de figer). Cold start Render Free **non tranché** — à décider après les tests marchands (mitigation keep-alive vs migration Railway/Vercel). **`render.yaml` non modifié** : il porte encore `read_orders` (scope inutilisé) dans `SHOPIFY_SCOPES`, mais c'est de la **config de déploiement production** → changement à valider explicitement (AGENTS.md §11) avant édition + `shopify app deploy`.
