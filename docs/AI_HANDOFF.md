...
- **Still open — `DATA_DIR` is ignored by two modules.** `app/jobs/audit_snapshot.py:27` and `app/api/deps.py:19-20` hardcode `Path(__file__).parents[2] / "data" / "raw"` instead of `data_dir()` from `app/paths.py`, whose docstring forbids exactly that. Observed on Render: the crawl reported writing to `/opt/render/project/src/data/raw/...` while `app/market_analysis/jobs.py` (which uses `data_dir()`) logs `/app/data/raw/...`, so the snapshot JSON lands off the mounted disk. It did not cause the empty catalog — the Postgres fallback in `snapshot_store.py` covered it — but the write is lost on redeploy.
