...
- **Date:** 2026-09-19
- **Agent:** Claude (Opus 5)
- **Goal:** Publish one blog article per day, picked for the traffic it can bring, tied to a product with its image and internal links.
- **What was missing:** blog ideas carried no traffic estimate at all. `build_blog_idea_suggestions` ordered them by a hard-coded angle priority (event > seasonal > trend > competitor > advantages) and the LLM's `proposed_blog_ideas` are just `{title, target_keyword, intro, outline}`. The `search_volume` already sitting on each product's `seo_keywords` was discarded by `_primary_keyword`, which keeps only the query string.
- **New `app/blog/traffic_score.py`:** scores each idea as `demand_per_day × CTR(position)`. Source priority is deliberate and matches Adrien's "real data first" rule — GSC impressions (confidence high), then DataForSEO volume (medium), then the `demand_score` bucket floor (low); every score carries `traffic_source` + `confidence`. A new article is assumed to reach position 8, or the query's current GSC position when it already ranks better. `load_gsc_index` reads the raw `gsc_*.json` because `_load_gsc_query_rows` drops the `days` window, and clicks *per day* is the whole point.
- **New `app/agent_schedule/daily_blog.py`:** picks the best unwritten idea, builds the draft, writes the sections, publishes **live** (Adrien's explicit call, not a hidden Shopify draft). Guards: `auto_apply` mode, the new `blog_article` scope, one article per day, no title already drafted or published, blog quota honoured via `check_quota`/`record_usage`. Never raises — a blog failure must not take the daily cycle down.
- **Image + internal links needed no new code:** `_draft_from_product` (`app/api/blog.py:258`) already picks the product's first image as the cover, builds the blog→product link via `build_source_product_link`, sets the CTA to the product URL and ties the article to the product through the `about` JSON-LD. The job reuses it rather than duplicating it.
- **Refactor:** `publish_draft_to_shopify` extracted from the `/publish` endpoint (now a thin wrapper) so the job reuses the exact same assembly — cover image, links, CTA, Article/FAQPage/HowTo JSON-LD, in-place update and the duplicate-article guard.
- **Opt-in:** `blog_article` is not in `DEFAULT_AUTO_PUBLISH_SCOPES`, so no other merchant starts publishing daily. Enabled in production for `287c4a-bb.myshopify.com` at Adrien's request.
- **Files created:** `app/blog/traffic_score.py`, `app/agent_schedule/daily_blog.py`, `tests/test_blog/test_traffic_score.py`, `tests/test_agent_schedule/test_daily_blog.py`.
- **Files modified:** `app/api/blog.py`, `app/agent_schedule/scheduler.py`, `tests/test_agent_schedule/test_scheduler.py`.
- **Validations:** `pytest` → 2291 passed / 174 skipped; `ruff check app tests` clean. 16 new tests covering source priority, position handling, tie-break order, GSC aggregation per query, and each publisher guard.
- **Not validated:** no daily cycle has run with this code — no article has actually been published to Shopify by the job. The first real run is the proof.
- **Known limits:** the CTR curve is a public-study average, not this niche's. On this shop DataForSEO returns volumes of 10 or None, so scores will be near zero and the ranking will lean on the demand bucket until GSC has data — the `confidence` field says so explicitly. Cannibalisation is not checked: an article can target a query the product page already ranks for.
...
- **Known gap (accepted):** the gating is UI-only. The locked questions' text still ships in the `market-analysis/latest` payload, and `POST /market-analysis/facts/{product_id}` enforces no per-plan quota. With the rolling unlock this matters less — a free shop can eventually answer everything, one question at a time; the paid value is doing it in one pass. Harden server-side only if this needs to become a real limit.
...
- **"No active products found" — diagnosed and fixed.** Confirmed against the dev store via Shopify CLI: all 17 products are `status: ACTIVE` with a valid `publishedAt` (2026-07-20), `onlineStoreUrl: null`, and a populated `onlineStorePreviewUrl` — the signature of a **password-protected storefront** whose products are genuinely published to the Online Store channel. `is_online_store_published` tested `onlineStoreUrl` first and returned on it, so `publishedAt` two lines below was never read and the whole catalog was filtered out. This is not a dev-store quirk: any merchant password-protecting the store before launch — precisely who installs an SEO app early — saw an empty catalog with no explanation. Fix (`app/snapshot/scope.py`): a present URL still proves publication; a **null** URL now falls through to `publishedAt`/`published_at` when that key exists, and only stays False when it is absent (so genuinely unlisted products, and the legacy no-signal path, are unchanged). `resourcePublications` would be the cleaner signal but needs the `read_publications` scope the app does not request — not worth an OAuth change. The crawl already fetches `publishedAt` (`scripts/audit/crawl_shopify.py:37`). Two tests added in `tests/test_snapshot/test_scope.py`.
- **Product identification labelled an English catalog in French (fixed).** Adrien: onboarding identified his English snowboard products correctly but named them in French ("snowboard léger et réactif"). `_build_label_prompt` (`app/market_analysis/identifier.py`) asked for "un label court **en français**" and never received the shop language — the caller passed none, and no `output_instruction` was appended. Its four examples were pet-shop only ("pull en cachemire pour chat", …). Fixed: the prompt takes `language`, drops the hardcoded French, appends `output_instruction(language)` per the repo convention (instructions stay French, output is parameterized), and the examples now span four unrelated verticals to teach the *form* rather than a sector. Caller passes `get_shop_language(shop_domain)`.
- **Competitor analysis was pinned to the French market (fixed, same pass).** `competitor_serp_engine.py` hardcoded `_LOCATION_CODE = 2250` (France) and `_LANGUAGE_CODE = "fr"` for every SERP lookup, so an English shop had its competitors resolved on google.fr. Now derived from `get_market(get_shop_language(shop))`. Its `_SYNTHESIS_SYSTEM` also declared "expert SEO/GEO **francophone** … en français" while the user prompt appended `language_context(language)` — a system prompt contradicting the language instruction. The language claim is gone from the system prompt; `language_context` is the single source.
- Tests: `tests/market_analysis/test_identifier_language.py` (9), including a niche-neutrality guard.
- **Paid shops got empty blog articles; free shops were fine (fixed).** Adrien: blog generation works on `surf-kvvjcg4x` (free) but produces titles with no content on `287c4a-bb` (agency), with Publish greyed out. Reproduced from the API: all three drafts on that shop had 3 sections, each with an `h2` and `direct_answer`/`body` of length **0**. Root cause: `generate_section` routes `GROUNDED_PLANS` (pro/agency) to Gemini + Google Search, and `app/llm/providers/gemini.py:60` deliberately omits `responseMimeType: application/json` when grounded — the API refuses search together with a forced JSON mime type. The call was nonetheless asked for JSON, so it answered in prose, `json.loads` raised, and the `except` returned the empty fallback for **every** section. Silent: the LLM call itself succeeds, so observability showed 21/21 successful calls and zero failures.
  - Fix (Adrien's call): the grounded call now asks for the section **in prose**, and a second cheap ungrounded call reshapes it into `{direct_answer, body, sources}` — the same split already proven in `app/niche/signals/realtime_trends._structure_events`. `claims_used` is left empty on the grounded path: the reshaping call must not invent claim-to-fact mappings. Grounding citations should now actually arrive, since `groundingMetadata` was empty only because of the forced JSON combination.
  - **Deliberately not fixed** (Adrien's call): a failed generation still produces a silent empty draft and still consumes the blog quota. Revisit if it bites again.
  - Tests: three rewritten/added in `tests/test_blog/test_section_generator.py`, including one that fails if a grounded prose answer yields an empty section.
- **Blog ideas were French-only and pet-flavored (fixed).** Same defect class as the enrichment questions below, in `app/blog/`. Two problems, not one: the copy was French on every store, **and the seasonal angle could never fire outside a French pet shop** — `_SEASONAL` matched product titles against French pet words ("manteau", "mue", "poil"), so an English catalog triggered nothing at all. New `app/blog/idea_templates.py` holds per-language season labels, trigger substrings and wording for all five angles (seasonal, trend, event, competitor, advantages) × FR/EN/DE/ES. Triggers are now everyday seasonal words ("winter", "cold", "warm") that match whatever the merchant sells. `build_blog_idea_suggestions` takes `language`; `app/api/blog.py` passes `get_shop_language`.
  - `app/blog/quality.py`: the four keyword-placement warnings are rendered raw in the draft UI (`app.blog.tsx:1605`) and were French — now localized, with `language` threaded through `_apply_keyword_check` at its three call sites.
  - `app/blog/schema.py`: `build_article_jsonld` defaulted to `language="fr"`, and the publish path (`app/api/blog.py:1157`) passed nothing — every published article declared `inLanguage: "fr"` regardless of the shop. Default now `DEFAULT_LANGUAGE`, call site passes the shop language.
  - Not changed: `section_generator.py` prompts are written in French but append `language_context(get_shop_language(shop))`, so article bodies should already follow the shop language. Worth confirming on a real English generation before assuming.
  - Tests: `tests/test_blog/test_idea_templates.py` (18) — seasonal angle fires in all four languages, wording follows the shop, key parity, and a guard that fails if pet vocabulary reappears.
- **Merchant enrichment questions were French-only and pet-flavored (fixed).** Adrien saw French questions under "Improve" on an English store. They are **not** LLM output — they were fixed templates hardcoded in French in `engine._build_enrichment_questions`, which never received the `language` already in scope at its call site. Worse, the examples were written for a pet-food catalog ("chiens adultes de petite race frileux", "chats d'intérieur séniors", "lapins nains"), absurd on a snowboard store and an obvious flag for App Store review. Moved to `app/market_analysis/enrichment_templates.py` (14 questions + placeholders + 5 rationale strings × FR/EN/DE/ES) rather than growing the 6k-line engine; the builder now takes `language`. Examples rewritten niche-neutral — deliberately generic rather than derived from the business profile, which would mean fabricating niche specifics.
  - Also removed the silent French fallback that hid this: five `language: str = "fr"` defaults in the engine now use `DEFAULT_LANGUAGE` (English, matching `app/language.py`), and the legacy `POST /market-analysis/run` now passes `get_shop_language(ctx.shop)` — it was the one call site that passed nothing and ran French for every shop. **Correction to an earlier read:** the main async path always threaded the language correctly (`app/api/market_analysis.py:769`); only the legacy endpoint did not.
  - Tests: `tests/market_analysis/test_enrichment_templates.py` (18), including key parity across the four locales and a guard that fails if pet vocabulary reappears in any template. Existing questions are frozen in saved analysis results — the change only shows after a re-analysis.
- **The app is now locked during onboarding.** Adrien's call: while onboarding is unfinished, no other page is visible or reachable, and returning to the app resumes on the exact step left off. Two obstacles were already in the code and had to be resolved first. (a) *Two divergent completion rules* — `app._index.tsx` redirected on `businessProfile.status !== "validated"` while `app.onboarding.tsx:97` also required a market analysis; between the two the dashboard rendered while onboarding thought itself unfinished, which is precisely what makes a route guard bounce. (b) *Embedded auth params* — `app._index.tsx:225-233` documented that redirecting without `shop`/`host`/`embedded`/`id_token` bounces to `/auth/login` (the "asks for shop domain" loop); the guard copies that pattern verbatim.
  - Backend `app/api/onboarding.py` (new, registered in `app/main.py`) is the single source of truth. `GET /onboarding/status` → `{complete, step}`; `complete` = validated profile **and** a market analysis. `step` = max(floor derived from the profile: none→1, unvalidated→2, validated→3; persisted step). `PUT /onboarding/step` records the furthest step reached, never backwards, stored in `shop_config` (Postgres-backed, survives redeploys) under `onboarding_step` — steps 4-6 leave no business trace that distinguishes "skipped Google" from "not there yet".
  - Guard in `shopify-app/app/routes/app.tsx`, the layout every `app.*` route renders through, so one check covers all of them. Fetched in parallel with the existing billing call. **Fails open** — a backend hiccup must never lock the merchant out. No route exceptions: Billing and Settings are blocked too (Adrien's call).
  - Removed both `redirectToOnboarding()` calls in `app._index.tsx`. The 404 one was a latent loop: a *completed* shop with a missing snapshot bounced to onboarding, which saw itself done and sent it straight back. Fresh installs are now covered by the guard.
  - Deliberately shipped with **no escape hatch** if the initial analysis never completes (Adrien's call: observe in testing rather than guess). Fail-open only covers an unreachable backend, not a job that hangs.
  - Tests: `tests/test_api/test_onboarding.py` (9). Note for future tests — `app/shop_config_store.py` binds `DB_PATH` at import, so patching `app.db_adapter.DB_PATH` alone leaves it writing to the real `data/history.db`; patch `app.shop_config_store.DB_PATH` too.
- **Merchants could never redo the product selection (fixed).** Adrien landed on Products with 3 products already picked — Gift Card first, which is meaningless for SEO — and no way to change them. Cause: the onboarding selection step showed an empty catalog (the bug above), so nothing was ever saved; `managed-products` still returns `selected_ids: null`, and `filter_managed_products` (`app/managed_products.py:88`) silently manages the first `cap` active products. The only existing control, "Add a product", is useless at 3/3 — it appends, it cannot swap. Added a **"Change my selection"** modal in `app.products.tsx`: checkbox list of every active product, cap enforced, pre-checked with the *effective* selection (`selected_ids ?? available.slice(0, cap)`, mirroring the backend fallback so the modal opens on what the app actually works on rather than on nothing). New `saveManagedSelection` action intent PUTs the selection, drops deselected products from the analysis via `market-analysis/products/remove`, and launches one grouped analysis for newly added products — same rule as `addManagedProduct`, where a change with no visible analysis reads as "nothing happened". The modal is mounted outside the step branches so it is reachable from identification and from analysis; entry points are a plain button on the identification card and one in `SummaryCard`. i18n FR/EN/DE/ES.
- **Diagnosis trail worth keeping:** the crawl itself was never the problem — `GET /api/shops/{shop}/jobs` showed `seo_audit` completed in 0.43 s with 17 products. The 90 s `CRAWL_TIMEOUT_MS` short-circuit theory was wrong.
- **`DATA_DIR` was ignored by six modules — fixed.** `render.yaml` runs a single Python service (`runtime: python`, code in `/opt/render/project/src`) with the persistent disk mounted at `/app/data` and `DATA_DIR=/app/data/raw`. Modules calling `data_dir()` wrote to the disk; modules hardcoding `Path(__file__).parents[N] / "data" / "raw"` wrote to the build directory, recreated on every commit (`autoDeployTrigger: commit`). Two distinct failures:
  - *Ephemeral snapshot.* `app/jobs/audit_snapshot.py` (writer) and `app/api/deps.py` (reader) hardcoded the **same** path, so they agreed with each other and the bug was invisible until a redeploy wiped it. Reads then fell back to Postgres, which is incomplete: `load_latest_snapshot_from_db` returns products/collections/pages/articles/redirects but **not the `shop` block**, so `_build_active_products` lost `primaryDomain.host` and GSC URL matching degraded until the next crawl. `app/shop_identity.py`, whose whole job is caching that host for exactly this case, hardcoded too and vanished with it.
  - *GSC CSVs written where nobody reads them.* `app/gsc/client.py:398,406` writes `gsc_performance.csv` / `gsc_query_page.csv` via `data_dir()` (the disk), while `_find_gsc_file` (`app/impact/report.py`, used by `app/api/shops.py:180` for visibility badges and by the impact report) and `_find_gsc_query_page_file` (`app/api/geo.py`) read the hardcoded root. Nothing self-heals: the import succeeds and every consumer reports no GSC data forever. Derived from code reading — not observed live, no Render shop has GSC connected.
  - Fix: all six resolve through `data_dir()` (`audit_snapshot.py`, `api/deps.py`, `shop_identity.py` as module constants, matching `gsc/client.py:36`; `impact/report.py`, `api/geo.py`, `geo/clicks_since_validation.py` inline, since they already resolved per call). Safe because `_DEFAULT_DATA_DIR` is byte-identical to the hardcoded expression when `DATA_DIR` is unset — local and test behavior is unchanged. No migration: after deploy the API reads `/app/data/raw`, finds nothing, falls back to Postgres, and the next crawl writes to the disk. Guarded by `test_no_module_hardcodes_the_raw_data_dir` in `tests/test_paths_and_scopes.py`, which fails on any new `"data" / "raw"` literal under `app/`.
- **Superseded hypothesis (kept for context):** The message shows whenever `available_products` is empty (`ProductSelectionPanel.tsx:169-173`), which conflates three causes indistinguishable to the merchant: (1) no snapshot at all — `load_snapshot_from_file_or_db` returns `None` → `[]` (`app/api/shops.py:164-167`); (2) a snapshot with zero ACTIVE products; (3) ACTIVE products not published to the Online Store channel, filtered by `is_active_online_store_product` (`app/snapshot/scope.py:59-62`). **Prime suspect is not the dev store:** onboarding step 1 stops polling the crawl after 90 s (`CRAWL_TIMEOUT_MS`, `OnboardingDiscoveryPanel.tsx:28,97-99`) and chains the analysis regardless — on Render Free with a cold start, reaching step 4 before `shopify_snapshot.json` is written is the nominal path, and the message then lies ("run the discovery first" — it did run, it just did not finish), with no way to re-run it from step 4. Diagnosis on `surf-kvvjcg4x.myshopify.com` is blocked on an authenticated Shopify read: the backend has no stored OAuth token for that shop (403 `is not installed` — expected, Remix injects the token per call), and the password-protected storefront blocks external probes. Shopify CLI upgraded to 4.5.2 for `shopify store execute`; waiting on Adrien's `shopify store auth`.
...
- **Fix:** all four now point at `localizedPath("/app/account", locale)`. A first attempt sent them to `/app/onboarding&step=3`, which did open the Google step but dropped the merchant into the onboarding wizard — Adrien rejected it: reconnecting must feel like the Settings button, not like redoing setup. `/app/account` renders `GoogleConnectionsCardWrapper`, which reconnects **in place** via a fetcher against the onboarding action, and is already what the SetupGuide's gsc/ga4 steps link to. Files: `app._index.tsx` (reauth + not-connected), `app.products.tsx` (same two).
...
- **Live results (2026-07-16 comparison export):** lever 1 works in production (2-4 `realtime_grounding` keywords per product; "fontaine à eau chat sans fil" entered via grounding, verified rising, became the product's #1 keyword targeted by the blog title). Lever 2 had no effect via prompt alone — Gemini still returned only ~6 verdicts per 30-keyword call and zero explicit `no_signal`. Lever 3 timid (one canicule blog idea on the fountain only; other products stayed evergreen — partly correct behavior).
- **Follow-up (same day):** `verify_keywords_against_market` now splits keywords into batches of `_VERIFY_BATCH_SIZE = 10` (max 3 grounded calls per product at the 30 cap), merging verdicts across batches; status "ok"/"partial"/error accordingly. Adrien accepted the up-to-×3 verify-call cost. Full pytest = 2146 passed (2 new batching tests).
- **Open:** re-run a live analysis to confirm verification coverage rises well above 6-8/product with batched calls. Also noted (not yet done): blog ideas "Guide : {mot-clé}" duplicates on the fountain look like a fallback template, worth investigating separately. Also noted (not yet done): the grounding prompts are hardcoded to the French market ("boutique française", "web francophone") — fine for now, must become dynamic before supporting non-FR shops.
...
- **Summary:**
  ...
  - **Frontend** : `app.analyse.tsx` → composant `ZeroClicksHint` (rendu sous `ClicksLine` pour produits ET blogs) : si `entry.total === 0` et `page_url`, affiche « Aucun clic — vérifiez l'indexation » + lien vers une recherche Google `site:<page_url>`. Clés i18n `analyseZeroClicksHint` / `analyseZeroClicksCheck` (FR+EN).
  - **Fix ouverture externe** : le lien Google s'ouvrait DANS l'iframe embarquée (App Bridge intercepte `<a target="_blank">` → Google refuse le framing « n'autorise pas la connexion »). Remplacé le `<Link external>` par un `<Button variant="plain" onClick={() => window.open(url, "_blank", "noopener,noreferrer")}>` (pattern déjà utilisé pour l'OAuth Google dans `app.onboarding.tsx` / `app.account.tsx`) → ouvre un vrai nouvel onglet.
