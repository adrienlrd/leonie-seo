# Guide utilisateur — GEO by Organically
...
GEO by Organically automatise l'audit SEO de votre boutique Shopify. Il :
...
> **Mode de distribution recommandé** pour les marchands Shopify : l'application GEO by Organically est embarquée dans l'admin Shopify (App Bridge + Polaris) via le scaffold Remix dans `shopify-app/`.
...
Oui. GEO by Organically est un outil auto-hébergé. Vos tokens d'accès et données produits ne quittent jamais votre environnement (CLI local ou serveur Docker).
