...
## État global
- Session 2026-06-13 : **Mode pilote supprimé — écritures Shopify toujours activées** — `is_pilot_safe_mode()` retourne `False` en dur (la variable d'env `LEONIE_PILOT_SAFE_MODE` n'a plus d'effet, non désactivable depuis l'app) ; garde-fou `confirm_live_write` conservé. Bannière dashboard + carte Réglages mode pilote retirées ; tests obsolètes (blocage mode pilote) supprimés. `ruff`/`typecheck`/`build` ✅, `pytest -q` 1870 passed / 182 skipped ✅.
- Session 2026-06-13 : **geo.py — lectures bloquantes uniformisées en `asyncio.to_thread`** — tous les endpoints async restants (`facts`, `priorities`, `weekly-actions`, `faq-content`, `risk-guard`, `collections`, `answer-blocks`, `crawlability`, `competitors`, POST `optimization-snapshots`) lisent désormais snapshot/CSV GSC via 3 helpers bloquants sous `to_thread`. Plus aucune I/O sync sur la boucle asyncio dans geo.py. Aucun impact marchand (endpoints hors hot-path UI), cohérence + filet de sécurité. `ruff` ✅, `pytest -q` 1874 passed / 185 skipped ✅.
- Session 2026-06-13 : **Dashboard — fiche produit complète (2 premiers)** — `ProductCard` + helpers extraits dans `components/ProductCard.tsx`, intents d'action mutualisés dans `lib/productCardActions.server.ts` (délégation depuis les deux routes), Accueil rend la fiche interactive complète pour les 2 premiers produits. Types partagés enrichis (champs optionnels). `typecheck`/`build` ✅.
- Session 2026-06-12 : **UX Produits — validation visible + décompte 28 j** — l'apply-to-shopify persiste `applied_fields` dans le pack (backend) ; côté UI : plus d'encadré jaune sur les champs validés, badge « Validé », décompte « Résultats dans {n} j » (J+28) puis lien vers Mesure, et `AnalysisLoader` sous « Régénérer avec mes réponses ». `ruff` ✅, `pytest test_market_analysis.py` 12 passed ✅, `typecheck`/`build` ✅.
- Session 2026-06-12 : **UX — loader unifié `AnalysisLoader`** — toutes les attentes longues (analyse marché, blog, onboarding ×3, audit dashboard, mono-produit, crawl concurrents) affichent désormais une barre de progression (progress réel ou rampe temporelle) + phrase vague rotative FR/EN (4 jeux dans `i18n.ts`). Remplace spinners isolés, barres figées à 50 % et textes bruts. `typecheck`/`build` ✅.
- Session 2026-06-12 : **Reconnexion Google visible** — la révocation du token Google (`invalid_grant`, app OAuth en mode Testing → tokens morts après 7 jours) échouait en silence : GA4 (`app/ga4/oauth.py`) crashait en 500, et la bannière dashboard se basait sur la présence du fichier GSC, pas sur le statut OAuth. Fix : flag `google_reauth_required` (shop_config) posé à la révocation (GSC + GA4), effacé à la reconnexion ; `gsc_status` expose `reauth_required` ; bannière 3 états (reconnexion critique / non connecté / rien) sur Accueil et Produits, basée sur `/gsc/status`. Action manuelle restante : passer l'app OAuth Google en « In production » (cause racine). `ruff` ✅, `pytest -q` 1873 passed ✅, `typecheck`/`build` ✅.
- Session 2026-06-12 : **Perf — navigation Produits/Blog** — `GET /products/active` (page Produits) lisait le snapshot 10-100 Mo synchrone dans un handler async (même anti-pattern que le 502 Mesure) → déplacé dans `asyncio.to_thread` ; loader Blog : 4 appels backend en série → liste brouillons + analyse marché parallélisés. `ruff` ✅, `pytest test_shops.py` 13 passed ✅, `typecheck`/`build` ✅.
- Session 2026-06-12 : **Perf — déduplication lecture snapshot sur cache froid** (`app/api/snapshot_store.py`) — suite du fix 502 : sur cache froid, `/geo/next-best-actions` et `/geo/control-groups` (appelés en parallèle par Mesure) relisaient chacun le snapshot (10-100 Mo), doublant le coût CPU/I/O. Ajout d'un verrou single-flight par chemin de fichier : un seul appelant lit+parse, les autres attendent puis réutilisent le cache. Comportement (TTL, invalidation mtime, fallback DB) inchangé. `ruff check` ✅, nouveau `tests/test_api/test_snapshot_store.py` (2 tests), `pytest -q` complet 1869 passed / 185 skipped / 0 failed ✅.
- Session 2026-06-12 : **Fix prod — 502 sur la page Mesure** (`app/api/geo.py`) — `/geo/next-best-actions` et `/geo/control-groups` lisaient le snapshot produit (10-100 Mo) de façon **synchrone** dans un handler `async def` ; appelés en parallèle par le loader Mesure sur cache froid, ils bloquaient la boucle asyncio ~54s, faisant échouer `/health` (timeout 5s, confirmé via Render Events) → Render tuait/redémarrait l'instance → 502 côté marchand. Fix : lecture snapshot + build déplacés dans `asyncio.to_thread`, même pattern que `run_continuous_improvement_agent`. `ruff check` ✅, `pytest tests/test_api/test_geo.py` 34 passed ✅. **Audit de suivi recommandé** : ~10 autres endpoints `geo.py` partagent le même anti-pattern (risque plus faible, appels non parallèles + cache 60s).
- Session 2026-06-12 : **Optimisation latence pré-lancement 50 marchands (sur Render actuel, indépendant de la migration Vercel/Railway/Supabase des tâches 169-171)** — sessions Shopify Postgres → Redis (`render.yaml` nouveau service `keyvalue` `leonie-seo-pilot-sessions`, `shopify.server.ts`) **déployées et confirmées en prod** (log `using Redis session storage`). Le réseau privé web↔api (`PYTHON_BACKEND_URL` via `fromService: hostport`) a été **revert** après régression en prod (`ENOTFOUND` — environnements Render pré-16/10/2025 en DNS IPv6-only, incompatibilité avec la résolution IPv4 par défaut de `fetch`/Node) ; `PYTHON_BACKEND_URL` repasse en `sync: false` (URL publique), descopé pour l'instant. Split du ticker `_agent_schedule_loop` en Background Worker dédié toujours écarté (snapshot produit sur disque local de l'API, non partageable — cf. `docs/AI_HANDOFF.md`). Budget confirmé : ~$24/mois.
...
- **2 — Liens morts corrigés + dashboard allégé** : remplacement de `/app/niche-understanding`, `/app/safe-apply`, `/app/impact`, `/app/priorities`, `/app/ga4` par des cibles valides (`/app/onboarding&step=2`, `/app/market-analysis?product=...` puis `/app/products?product=...` après la tâche 3, `/app/measure`). `app.onboarding.tsx` accepte un paramètre `?step=` pour revenir manuellement à une étape sans boucle de redirection. Détails : `docs/AI_HANDOFF.md`.
...
- `cd shopify-app && npm run typecheck` ✅, `npm run build` ✅ (après chaque tâche).
- `code-reviewer` exécuté sur chaque diff.
- `ruff check .`/`pytest` non requis (aucun changement Python sur ces 4 tâches, hors `app/api/ga4.py` à la tâche 1).
