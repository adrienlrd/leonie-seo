...
## État global
- Session 2026-06-12 : **Optimisation latence pré-lancement 50 marchands (sur Render actuel, indépendant de la migration Vercel/Railway/Supabase des tâches 169-171)** — sessions Shopify Postgres → Redis (`render.yaml` nouveau service `keyvalue` `leonie-seo-pilot-sessions`, `shopify.server.ts`) pour éviter un aller-retour Postgres à chaque navigation marchand (`authenticate.admin()`), et `PYTHON_BACKEND_URL` web↔api basculé sur le réseau privé Render (`fromService: hostport` + normalisation de schéma dans `api.server.ts`). `npm run typecheck` ✅, `npm run build` ✅. Reste à faire (manuel, dashboard Render) : sync du Blueprint pour provisionner `leonie-seo-pilot-sessions` (~+$10/mois) puis déployer. Split du ticker `_agent_schedule_loop` en Background Worker dédié écarté pour l'instant (snapshot produit sur disque local de l'API, non partageable — cf. `docs/AI_HANDOFF.md`).
